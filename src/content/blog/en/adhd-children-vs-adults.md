...
- **The "onset before 12" criterion gets fuzzy in retrospect.** Adults often can't reconstruct childhood symptoms with confidence, and parents' memories are filtered. We covered the diagnostic mechanics in [ADHD: DSM-5 diagnostic criteria, explained plainly](/en/blog/adhd-dsm-5-criteria/) — the short version is that retrospective history is accepted, but it's harder to make the case than for someone with school records full of red flags.
...
What helps at this age: structure imposed from outside, clear short-term feedback, movement breaks, parents and teachers who understand the brain isn't being defiant. Medication, when prescribed, is most often a stimulant — methylphenidate or amphetamine-based — and the [CHADD parent resources](https://chadd.org/for-parents/overview/) are a solid starting place for families.
...
This is the version that DopaHop was built for. If you're trying to start a task and can't, the [Pomodoro timer](/#cosa-fa) handles the "press play and the timer carries you" piece — the friction of beginning often vanishes once a structure is running, even a small one.
...
- The [Pomodoro timer](/#cosa-fa) for the task-initiation piece — press start, the timer runs, you don't have to think about it.
- The [brain dump](/#cosa-fa) for the working memory leak — when a thought appears and you know you'll lose it in 30 seconds, you have ten seconds to capture it instead.
- The [task breakdown wizard](/#cosa-fa) for when "do the taxes" is too big to start — you walk through it once, and now it's "open the folder," "find last year's return," "log into the portal." Each step is small enough that the brain doesn't refuse.
